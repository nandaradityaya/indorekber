...
class MoneyExchangeController extends GetxController
  ...
  void calculateExchangeRate() {
    exchangeRate.value =
        toSelectedCurrencyRate.value / fromSelectedCurrencyRate.value;
    min.value = fromSelectedCurrencyRate.value *
        moneyExchangeModel.data.charges.minLimit;
    max.value = fromSelectedCurrencyRate.value *
        moneyExchangeModel.data.charges.maxLimit;

    fCharge.value = moneyExchangeModel.data.charges.fixedCharge *
    ...
    if (fromAmountController.text.isNotEmpty) {
      ...
      toAmountController.text = (amount * exchangeRate.value)
          .toStringAsFixed(toSelectedCurrencyType.value == "FIAT" ? 2 : 6);
      pCharge.value =
          (amount / 100) * moneyExchangeModel.data.charges.percentCharge;
